...
export default class AssetLoader2 {
	...
	static CSS(src: string): HTMLStyleElement {
		const e = AssetLoader2.Get("style", src)
		if (!e) throw new Error("Could not find CSS from preloaded CSS. (" + src + ")")

		if (e instanceof HTMLStyleElement) {
			const copy = <HTMLStyleElement>e.cloneNode(false) // do not deep copy
			return copy
		} else {
			throw new Error("`style` in `greenframe-resource-preload` is wrong internal type. (" + src + ")")
		}
	}
}
